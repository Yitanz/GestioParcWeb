...
<nav class="navbar navbar-expand-sm bg-dark navbar-dark py-4">
  ...
  <div class="collapse navbar-collapse" id="collapsibleNavbar">
    <ul class="navbar-nav">
      ...
      <li class="nav-item dropdown">
        ...
        <ul class="dropdown-menu" aria-labelledby="navbarDropdownMenuLink">
          <li><a class="dropdown-item" href="/ticket/elegirTicket.php">Parc</a></li>
  ...
  <div class="container">
    <div class="row">
      ...
      <tfoot>
          <tr>
              <td><a onclick="window.history.back();" class="btn btn-warning"><i class="glyphicon glyphicon-menu-left"></i>Segueix comprant</a></td>
              <td colspan="2"></td>
              <?php if($cart->total_items() > 0){ ?>
              <td class="text-center"><strong>Total <?php echo $cart->total().' €'; ?></strong></td>
              <td><a href="formulari_compra.php" class="btn btn-success btn-block">Pagar<i class="glyphicon glyphicon-menu-right"></i></a></td>
              <?php } ?>
          </tr>
      </tfoot>
      </table>
...
if(!empty($_POST)){
  ...
  $xiquet = $_POST['xiquet'];
  $nado = $_POST['nado'];
  $visitant = $_POST['visitant'];
  var_dump($adult);

  if($adult == 0 && $xiquet == 0 && $visitant == 0 && $nado == 0){
    echo'<script>
    alert("Tens que afegir tickets.");
    </script>';
    header('url=/ticket/ticket.php');
  }else{
    echo '<script>
            alert("Tickets afegits a la cistella.");
          </script>';
  }
}
 ?>
