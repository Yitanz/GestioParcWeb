...
<a href="viewCart.php" class="cart-link" title="View Cart"><i class="glyphicon glyphicon-shopping-cart"></i></a>
  <div id="products" class="row">
      ...
      //get rows query"
      $comparar = "ticket_general";
      $query = $connection->query("SELECT * FROM products WHERE tipus_producte = 'ticket_general' ORDER BY id DESC LIMIT 10");
      if($query->num_rows > 0){
          while($row = $query->fetch_assoc()){

        echo '<div class="col-md-3 jumbotron">';
        echo '  <h4 class="list-group-item-heading">'.$row["name"].'</h4>';
        echo '  <p class="list-group-item-text">'. $row["description"] .'</p>';
        echo '  <div class="col-md-6">';
        echo '    <p class="lead">'.$row["price"].' €</p>';
        echo '  </div>';
        echo '  <div class="col-md-6">';
        echo '    <a class="btn btn-success" href="../php/cartAction.php?action=addToCart&id='. $row["id"] .'">Afegir a la cistella</a>';
        echo '  </div>';
